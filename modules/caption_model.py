...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        def beam_step(logprobs, unaug_logprobs, beam_size, t, beam_seq, beam_seq_logprobs, beam_logprobs_sum, state):
            ...
            beam_ix = torch.div(ix,vocab_size,rounding_mode='trunc')#kameradna  # Nxb which beam
        ...
        for t in range(self.max_seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.max_seq_length + divm - 1:
                    ...
                    # suppress previous word
                    if decoding_constraint and t - divm > 0:
                        logprobsf.scatter_(1, beam_seq_table[divm][t - divm - 1].unsqueeze(1).cuda(), float('-inf'))#flag to unimplement cuda for multiprocessing, cuda() does not necessarily mean the home GPU
                    # suppress UNK tokens in the decoding
                    ...
                    logprobs_table[divm], state_table[divm] = self.get_logprobs_state(it.cuda(), *(#flag to unimplement cuda for multiprocessing, cuda() does not necessarily mean the home GPU
                            args[divm] + [state_table[divm]]))
                    logprobs_table[divm] = F.log_softmax(logprobs_table[divm] / temperature, dim=-1)
    ...
    def sample_next_word(self, logprobs, sample_method, temperature):
        if sample_method == 'greedy':
            ...
            def sample_gumbel(shape, eps=1e-20):
                U = torch.rand(shape).cuda()#flag to unimplement cuda for multiprocessing, cuda() does not necessarily mean the home GPU
